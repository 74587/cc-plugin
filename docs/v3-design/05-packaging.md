...
├── agents/                         # investigator.md / reflector.md / recorder.md(角色契约)
├── hooks/
│   └── hooks.json                  # SessionStart/Stop/PreCompact → npm alias → scoped llmdoc hook *
...
├── .codex/agents/                  # ACPlugin 生成的三个 project-scoped agent 契约
├── docs/
│   └── v3-design/                  # 本设计
...
- V2 遗留的 worker 与 tracked reflection/memory 树、`skills/llmdoc/references/` 大部头等废弃；受限 Reflector 只写 `.llmdoc-tmp/reflections/pending/`。Codex 角色文件只接受 ACPlugin 从三个 Claude 角色契约重生成。
...
4. Codex 当前支持 project-scoped custom agents；保留 ACPlugin 从三个 Claude 角色契约生成的 `.codex/agents/*.toml`,但不手工维护第二份角色文本,也不在安装后另行改写用户项目;
...
3. **Prompt 重写**:operating + init/update SOP + 三个 agent 契约,在本仓库 dogfood(init 自举);

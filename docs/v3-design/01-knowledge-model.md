...
正确、可验证不等于值得长期保存。`llmdoc/` 的正文不是源码、CLI help、schema、测试或调查报告的副本;它优先保存遗忘后会让未来 agent 做出不同且错误选择的知识。

Recorder 在新增或保留一条稳定陈述前,必须同时检查:

1. **决策影响**:读到它会改变未来的设计、实现、操作或 review 选择;
2. **恢复成本**:无法在几分钟内从源码、schema、CLI help、测试或生成配置中可靠恢复;
3. **持续性**:预期能跨过日常 commit,而不是只描述当前 release、文件库存或本次调查;
4. **唯一归属**:有清晰的 canonical 文档归属,且正文增加了理由、边界、后果或跨文件关系,而不是只重复路径指针。

稳定正文的优先级依次是:决策及理由 → 架构边界与不变量 → 跨模块设计契约与失败语义 → 高风险可重复工作流 → 少量非显然实现导航。原始证据只负责建立可信度,不负责占据正文:

| 内容 | 归属 |
|---|---|
| 决策、理由、边界、不变量、后果 | `llmdoc/` 正文 |
| `code.paths`、relations、少量 canonical anchor | front matter / 正文短引用 |
| 精确字段、命令和文件库存 | 源码、schema、CLI help、测试 |
| 调查输出、计数、逐 commit 事实 | `.llmdoc-tmp/` |

过渡性事实只有在遗漏会导致不安全操作时才能进入正文,并必须写明退出条件。代码变化或 `delta` 命中只产生**复核义务**,不自动产生正文写入义务;若原文语义仍成立,Recorder 应标记 verified unchanged,只推进有效性锚点,不得把新证据追加成正文。

## 8. 写作规范(对 AI 友好的三条硬约束)
